...
function SourceCard({ fuente, cfg, onLeftChange, onTurnoutChange }) {
  ...
  return (
    <div className={`source${fuente.wide ? " full" : ""}`}>
      <div className="sigla source-sigla" style={{ color: fuente.color }}>
        {fuente.sigla}
      </div>
...
export default function SimuladorBalotajeBeni() {
  ...
  const resultado = useMemo(() => {
    const baseIzquierdaRetenida = Math.round(
      FINALISTAS.izquierda.votos * (retencionIzquierda / 100),
    );
    const baseDerechaRetenida = Math.round(
      FINALISTAS.derecha.votos * (retencionDerecha / 100),
    );
  ...
  * { box-sizing: border-box; }
  ...
  .sigla { font-size: 22px; font-weight: 800; }
      ...
      <div className="wrap">
              ...
              <div className="row-between small muted" style={{ fontWeight: 800 }}>
                ...
                <span>
                  {resultado.ganador === "EMPATE"
                    ? "Empate técnico"
                    : `Ventaja: ${fmtInt.format(Math.abs(resultado.margen))} votos`}
                </span>
              </div>
            ...
            <div className="ballot-box">
              <div className="row-between small muted" style={{ fontWeight: 800 }}>
                ...
                <span>
                  {resultado.ganador === "EMPATE"
                    ? "Empate técnico"
                    : `Ventaja: ${fmtInt.format(Math.abs(resultado.margen))} votos válidos`}
                </span>
              </div>
              <div className="ballot-grid" style={{ marginTop: 12 }}>
                <div>
                  <div className="small" style={{ fontWeight: 800, color: "#f97316" }}>
                    PATRIA-UNIDOS
                  </div>
                  <div className="ballot-big" style={{ color: "#f97316" }}>
                    {fmtPct(resultado.izquierdaBalotajePct)}
                  </div>
                </div>
                <div>
                  <div className="small" style={{ fontWeight: 800, color: "#ec4899" }}>
                    MNR
                  </div>
                  <div className="ballot-big" style={{ color: "#ec4899" }}>
          ...
          <div className="card">
            <div className="row-between">
              ...
              <div
                ...
                style={{
                  background:
                    resultado.ganador === FINALISTAS.derecha.sigla
                      ? FINALISTAS.derecha.color
                      : resultado.ganador === FINALISTAS.izquierda.sigla
                        ? FINALISTAS.izquierda.color
                        : "#71717a",
                }}
              >
                {resultado.ganador}
            ...
            <div className="stats-grid" style={{ marginTop: 14 }}>
              ...
              <div className="mini-box">
                ...
                <div className="small">
                  {resultado.margen === 0 ? "Empate" : `a favor de ${resultado.ganador}`}
                </div>
              </div>
            </div>

            <div className="dashed tiny muted" style={{ marginTop: 14 }}>
              El simulador parte desde la primera vuelta real: 36,56% para PATRIA-UNIDOS y
              20,12% para MNR. En el bloque superior, el cómputo final del balotaje se
              recalcula solo sobre los votos que terminan en uno de los dos finalistas.
            </div>
          ...
          {FUENTES.map((fuente) => (
            <SourceCard
              ...
              onLeftChange={(left) =>
                setFuentes((prev) => ({
                  ...prev,
                  [fuente.key]: { ...prev[fuente.key], left },
                }))
              }
              onTurnoutChange={(turnout) =>
                setFuentes((prev) => ({
                  ...prev,
                  [fuente.key]: { ...prev[fuente.key], turnout },
                }))
              }
            />
          ))}
        ...
        <div className="card" style={{ marginTop: 16 }}>
          <div className="footer-note">
            Votos válidos primera vuelta: <b>{fmtInt.format(VOTOS_VALIDOS)}</b> · Blancos:{" "}
            <b>{fmtInt.format(VOTOS_BLANCOS)}</b> · Nulos: <b>{fmtInt.format(VOTOS_NULOS)}</b>
          </div>
          <div className="footer-note" style={{ marginTop: 8 }}>
            Emitidos: <b>{fmtInt.format(VOTOS_EMITIDOS)}</b> · Ausentes:{" "}
            <b>{fmtInt.format(AUSENTES)}</b> · Padrón: <b>{fmtInt.format(PADRON)}</b>
          </div>
        </div>
      </div>
    </>
